...
export function detectPatterns(chart: FunctionalAstrolabe): Pattern[] {
  ...
  if (['七殺', '破軍', '貪狼'].some((s) => soulMajors.has(s))) {
    patterns.push({
      name: '殺破狼',
  ...
  if (['天機', '太陰', '天同', '天梁'].every((s) => trineMajors.has(s))) {
    patterns.push({
      name: '機月同梁',
  ...
  for (const p of trinePalaces) {
    const names = allNames(p)
    if (names.includes('貪狼') && names.includes('火星')) {
      patterns.push({
        name: '火貪格',
  ...
  for (const p of trinePalaces) {
    const names = allNames(p)
    if (names.includes('貪狼') && names.includes('鈴星')) {
      patterns.push({
        name: '鈴貪格',
  ...
  if (soulMajors.has('太陽') && soulMajors.has('太陰')) {
    patterns.push({
      name: '日月同宮',
  ...
  if (['太陽', '天梁', '文昌', '祿存'].every((s) => trineAll.has(s))) {
    patterns.push({
      name: '陽梁昌祿',
  ...
  for (const p of chart.palaces) {
    const names = allNames(p)
    if (names.includes('天馬') && luckStars.some((s) => names.includes(s))) {
      patterns.push({
        name: '祿馬交馳',
  ...
  if (soulMajors.has('巨門') && ['子', '午'].includes(soul.earthlyBranch)) {
    patterns.push({
      name: '石中隱玉',
